...
class MSOModule(object):
    ...
    def __init__(self, module):
        ...
        self.module.params['url_username']=module.params.get("username")
        self.module.params['url_password']=module.params.get("password")
        ...
        if self.module._socket_path is None:
            ...
            if self.connection.get_platform() == "cisco.nd":
                self.platform = "nd"
                # self.connection.queue_message('vvvv', 'logs from MSOModule send_request method called')
                # self.fail_json(msg="entered to the else part of the connection socket")
    ...
    def request_download(self, path, destination=None):
        ...
        data = None
        # response_obj = self.connection.send_file_request("GET", self.url, file="ansibleBackup2", data=None)
        # response_obj = self.connection.send_request("GET", self.url,  data=None)

        # self.get_obj(path)
        # self.fail_json(msg="socket_path _ansible_socket: {0}, _socket_path: {1}".format(self.params.get("_ansible_socket"), self.module._socket_path))

        kwargs = {}
        if destination is not None:
            if os.path.isdir(destination):

                # self.fail_json("Module: {0}, URL: {1}, Headers: {2}".format(self.module.__dict__,
                # self.url,
                # self.headers))
                # first check if we are redirected to a file download

                # fetch_file_return_val = fetch_file(self.module, self.url, data=None, headers=self.headers, method="GET",
                #            use_proxy=True, force=False, last_mod_time=None, timeout=10)
                #
                # self.fail_json("fetch_file_return_val: {0}".format(fetch_file_return_val))

                # Set params to access send_request(self, method, path, data=None)

                send_request_return_val = self.connection.send_request("POST", "/mso/api/v1/"+path, data=None)
                # send_request_return_val.pop("raw")
                # self.fail_json("send_request_return_val: {0}".format(send_request_return_val))
                # self.stdout = "{0} , New value added: {1}".format(self.stdout, send_request_return_val)
                # self.fail_json(msg=self.stdout)
                destination = os.path.join(
                    destination,
                    send_request_return_val.get("content-disposition").split("filename=")[1],
                )

                write_file(self.module, self.url, destination, send_request_return_val.get("raw"), "resp")

                self.fail_json(msg="after the write file")
                # # send_file_request
                ...
                #     method="GET",
                #     timeout=self.params.get("timeout"),
                #     unix_socket=self.module._socket_path,
                #     # url_username=self.params.get('username'),
                #     # url_password=self.params.get('password'),
                #     **kwargs
                # )
                # self.fail_json("check: {0}, redir_info: {1}".format(check, redir_info))
        ...
        self.stdout = "{0} , New value added destination: {1}, kwargs: {1}".format(self.stdout, destination, kwargs)
    ...
    def request_upload(self, path, fields=None):
        ...
        self.path = path
        # self.url ="https://173.36.219.31/mso/api/v1/backups/remoteUpload/63a018e7190000587d6e5f77"
        uri ="/mso/api/v1/backups/remoteUpload/63a018e7190000587d6e5f77"

        # self.url = urljoin(self.baseuri, path)
        path="/backups/remoteUpload/63a018e7190000587d6e5f77"
        file = "/Users/sajagana/Downloads/ansibleBackup1-20230106081100.tar.gz"

        # Using the connection socket
        send_file_request_return = self.connection.send_file_request("POST", uri, file=file)
        self.httpapi_logs.extend(self.connection.pop_messages())

        self.stdout = "send_file_request_return: {0}".format(send_file_request_return)
        self.fail_json(msg=self.stdout)
        ...
        self.module.params["url_username"] = "admin"
        self.module.params["url_password"] = "ins3965!"
        resp, info = fetch_url(
            self.module,
            self.url,
            data=mp_encoder,
            headers=self.headers,
            method="POST",
            unix_socket=self.connection.socket_path,
            timeout=100,
            use_proxy=False,
        )
        ...
        self.stdout = "response: {0}, status: {1}".format(self.response, self.status)
        # backup_file_name = fields.get("name")[0]
        # send_file_request_return_value = self.connection.send_file_request("POST", self.url, file=backup_file_name, data={"a":1})
        # self.stdout = "backup_file_name: {0}, {1}".format(backup_file_name, send_file_request_return_value)
        # import inspect
        # self.stdout = "backup_file_name: #########: {0}, #######: {1}, #######: {2}, #######: {3}, #######: {4}".format(self.connection.__dict__, dir(self.connection), inspect.getsourcelines(self.connection.send),inspect.getsourcefile(self.connection.send), self.connection)
        # self.fail_json(msg="failed")
        # response = self.connection.send(mp_encoder)

        # self.stdout = "backup_file_name: response: {0}".format(response)

        # response = self.connection.send_file_request( "POST", self.url, "./"+backup_file_name, mp_encoder)
