...
options:
  ...
  security_policy:
    ...
    type: str
    choices: [ default, elb_sec_2016_18, elb_sec_fs_2018_06, elb_sec_tls_1_2_2017_01, elb_sec_tls_1_2_ext_2018_06, elb_sec_tls_1_1_2017_01, elb_sec_2015_05, elb_sec_tls_1_0_2015_04, app_gw_ssl_default, app_gw_ssl_2015_501, app_gw_ssl_2017_401, app_gw_ssl_2017_401s ]
  ssl_certificates:
    description:
    ...
    suboptions:
      ...
      health_check:
        ...
        suboptions:
          ...
          host:
            description:
            - The host of the health check. The host attribute will be enabled when the O(use_host_from_rule) is false.
            type: str
          success_code:
            description:
...
def main():
    ...
    if state == "present":
        ...
        if device is None and parent_present is False:
            mso.fail_json(msg="The 'device' name is required to initialize the parent object.".format(device))
            ...
            if (query_device_data.get("deviceVendorType") == "NATIVELB" and query_device_data.get("devType") == "application") or (
                ...
                cloud_service_nodes_relationship = {}
        ...
        for position, rule in enumerate(rules, 0):
            ...
            if health_check:
                ...
                if listener_protocol in ["tcp", "udp"]:
                    ...
                    mso.fail_json(
                        msg="The 'listener_protocol': {0} and the health_check protocol: {1} is not a valid configuration at the object position: {2} and the object name: {3}".format(
                            listener_protocol, health_check.get("protocol"), position, rule.get("name")
                        )
                    )
        ...
        if parent_present:
            ...
            if mso.existing:
                listener_path = "/sites/{0}-{1}/contracts/{2}/serviceGraphRelationship/serviceNodesRelationship/{3}/deviceConfiguration/cloudLoadBalancer/listeners/{4}".format(
                    site_id, template, contract, service_node_index, listener
                )
                op = "replace"
            else:
                # Create a new listener
                listener_path = "/sites/{0}-{1}/contracts/{2}/serviceGraphRelationship/serviceNodesRelationship/{3}/deviceConfiguration/cloudLoadBalancer/listeners/-".format(
